  ...
  return (
    <div style={{ marginTop: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.625rem' }}>
        <p style={{ fontSize: '0.75rem', color: 'rgba(246,247,248,0.4)', fontFamily: 'Anton, Impact, sans-serif', letterSpacing: '0.06em' }}>
          CONNECTED APPS
        </p>
        {sessions.length > 1 && (
          <button
            onClick={disconnectAll}
            style={{ background: 'none', border: 'none', fontSize: '0.75rem', color: 'rgba(246,247,248,0.4)', cursor: 'pointer', textDecoration: 'underline', padding: 0 }}
          >
            Disconnect all
          </button>
        )}
      ...
      {sessions.length === 0 && (
        <div className="card-md" style={{ textAlign: 'center', padding: '1.5rem' }}>
          <p style={{ fontSize: '0.875rem', color: 'rgba(246,247,248,0.3)' }}>No apps connected</p>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
        {sessions.map((session) => {
          ...
          const iconSrc = peer?.icons && peer.icons.length > 0 ? peer.icons[0] : null
          const url = peer?.url ?? ''
          ...
          return (
            ...
            >
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', fontSize: '0.75rem', color: 'var(--teal)', cursor: 'pointer', textDecoration: 'underline', flexShrink: 0, padding: 0 }}
              >
                Disconnect
              </button>
            </div>
